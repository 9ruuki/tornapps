...
type TravelRow = { destination: Destination; tripTime: number; margin: number; cashRequired: number; roiPercent: number; profit: number; profitHour: number; tripsPerDay: number; dailyProfit: number; activity: string; score: number };
...
function App() {
  ...
  const travelRows: TravelRow[] = useMemo(() => destinations.map(destination => {
    ...
    const cashRequired = buy * capacity;
    ...
    const score = profitHour * Math.max(0.15, Math.min(1, roiPercent / 20));
    return { destination, tripTime, margin, cashRequired, roiPercent, profit, profitHour, tripsPerDay, dailyProfit, activity: activityLabel(tripsPerDay), score };
  }).sort((a, b) => b.score - a.score), [capacity, dailyMinutes, margin, buy, stay]);
  ...
  function loadDemoTravel() { updateTravel("destination", "mexico"); updateTravel("capacity", 19); updateTravel("buy", 500); updateTravel("sell", 2500); updateTravel("stay", 4); updateTravel("dailyMinutes", 240); log("travel_demo", "loaded demo travel values"); }
  ...
  return <div className="app"><aside className="sidebar"><div className="brand"><Logo /><div><strong>TornApps</strong><span>{tr(locale, "Checkpoint R7", "Checkpoint R7")}</span></div></div><nav>{tools.map(x => <button key={x.id} className={active === x.id ? "active" : ""} onClick={() => setTool(x.id)}><b>{x.icon}</b><span>{x[locale]}</span><small>{x[`desc${locale === "en" ? "En" : "Es"}` as "descEn"]}</small></button>)}</nav></aside><main className="main"><header><div><p className="eyebrow">{tool[locale]}</p><h1>{tr(locale, "Travel decisions with real numbers.", "Decisiones de viaje con numeros reales.")}</h1><p className="subtle">{tr(locale, "R7 adds QA metrics: cash required, ROI, score, demo data and saved-plan visibility.", "R7 anade metricas QA: capital requerido, ROI, score, demo y visibilidad de planes.")}</p></div><button className="btn" onClick={toggleLocale}>{locale.toUpperCase()}</button></header><section className="tiles"><Tile label="Key" value={customReady ? "Custom OK" : "Not ready"} /><Tile label="Best route" value={bestRow.destination.en} /><Tile label="Best score" value={bestRow.score.toFixed(0)} /><Tile label="Plans" value={String(plans.length)} /></section>{active === "overview" && <Panel title="Checkpoint R7" subtitle={tr(locale, "The Travel module now has enough calculation depth to be visually QA'd after deployment.", "El modulo Viajes ya tiene suficiente profundidad para revision visual tras deploy.")}><TravelSummary locale={locale} row={bestRow} /><CheckList probes={probes} /></Panel>}{active === "api" && <Panel title="Custom API Key" subtitle={tr(locale, "Create a Custom key named TornApps, copy the permissions checklist, then validate it here.", "Crea una key Custom llamada TornApps, copia la lista de permisos y validala aqui.")}><div className="actions"><a className="btn primary" href={TORN_API_PAGE} target="_blank" rel="noreferrer">{tr(locale, "Create Custom Key on Torn", "Crear Custom Key en Torn")}</a><button className="btn" onClick={() => void copyRequirements()}>{tr(locale, "Copy required permissions", "Copiar permisos requeridos")}</button></div><input className="input" type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Paste Custom API key" /><div className="actions"><button className="btn primary" onClick={() => void validateCustomKey()}>{tr(locale, "Validate Custom Key", "Validar Custom Key")}</button><button className="btn danger" onClick={clearData}>{tr(locale, "Clear local data", "Borrar datos locales")}</button></div><Notice status={apiStatus} text={message} /><CheckList probes={probes} /></Panel>}{active === "travel" && <Panel title={tr(locale, "Travel ROI Planner", "Planificador ROI de viajes")} subtitle={tr(locale, "Compare every destination by score, profit per hour, ROI, daily profit and activity level.", "Compara destinos por score, beneficio/hora, ROI, beneficio diario y actividad.")}><div className="actions"><button className="btn" onClick={loadDemoTravel}>{tr(locale, "Load demo values", "Cargar demo")}</button></div><div className="form"><select className="input" value={destinationId} onChange={e => updateTravel("destination", e.target.value)}>{destinations.map(d => <option key={d.id} value={d.id}>{locale === "en" ? d.en : d.es} - {d.minutes}m</option>)}</select><input className="input" type="number" value={capacity} onChange={e => updateTravel("capacity", e.target.value)} placeholder="Capacity" /><input className="input" type="number" value={buy} onChange={e => updateTravel("buy", e.target.value)} placeholder="Buy price" /><input className="input" type="number" value={sell} onChange={e => updateTravel("sell", e.target.value)} placeholder="Sell price" /><input className="input" type="number" value={stay} onChange={e => updateTravel("stay", e.target.value)} placeholder="Extra minutes per trip" /><input className="input" type="number" value={dailyMinutes} onChange={e => updateTravel("dailyMinutes", e.target.value)} placeholder="Available minutes/day" /></div><section className="tiles"><Tile label="Margin/item" value={money(margin)} /><Tile label="Cash needed" value={money(selectedRow.cashRequired)} /><Tile label="ROI/trip" value={pct(selectedRow.roiPercent)} /><Tile label="Profit/hour" value={money(selectedRow.profitHour)} /></section><TravelSummary locale={locale} row={selectedRow} /><div className="actions"><button className="btn primary" onClick={() => savePlan(selectedRow)}>{tr(locale, "Save selected plan", "Guardar plan seleccionado")}</button><button className="btn" onClick={() => savePlan(bestRow)}>{tr(locale, "Save best route", "Guardar mejor ruta")}</button></div><TravelTable locale={locale} rows={travelRows} selectedId={selected.id} onSelect={(id) => updateTravel("destination", id)} /><SavedPlans plans={plans} /></Panel>}{active === "history" && <Panel title="History" subtitle="Local events.">{events.map(e => <div className="rowline" key={e.id}><strong>{e.type}</strong><span>{new Date(e.time).toLocaleString()}</span><p>{e.message}</p></div>)}</Panel>}{active === "settings" && <Panel title="Settings" subtitle="Local privacy controls."><button className="btn danger" onClick={clearData}>Delete local data</button></Panel>}<nav className="mobile">{tools.map(x => <button key={x.id} className={active === x.id ? "active" : ""} onClick={() => setTool(x.id)}>{x.icon}</button>)}</nav></main></div>;
}
...
function TravelSummary({ locale, row }: { locale: Locale; row: TravelRow }) { return <div className="decision-card"><div><span>{tr(locale, "Recommendation", "Recomendacion")}</span><strong>{locale === "en" ? row.destination.en : row.destination.es}</strong><p>{tr(locale, "Ranking uses profit/hour weighted by ROI so high capital traps are easier to spot.", "El ranking usa beneficio/hora ponderado por ROI para detectar trampas de capital.")}</p></div><div className="decision-metrics"><Tile label="Trips/day" value={row.tripsPerDay.toFixed(1)} /><Tile label="Daily profit" value={money(row.dailyProfit)} /><Tile label="Activity" value={row.activity} /></div></div>; }
function TravelTable({ locale, rows, selectedId, onSelect }: { locale: Locale; rows: TravelRow[]; selectedId: string; onSelect: (id: string) => void }) { return <div className="travel-table"><div className="table-head"><span>Destination</span><span>Score</span><span>$/h</span><span>ROI</span><span>Daily</span></div>{rows.map(row => <button key={row.destination.id} className={row.destination.id === selectedId ? "selected" : ""} onClick={() => onSelect(row.destination.id)}><span>{locale === "en" ? row.destination.en : row.destination.es}</span><span>{row.score.toFixed(0)}</span><span>{money(row.profitHour)}</span><span>{pct(row.roiPercent)}</span><span>{money(row.dailyProfit)}</span></button>)}</div>; }
function SavedPlans({ plans }: { plans: TravelPlan[] }) { if (!plans.length) return null; return <div className="saved-plans"><h3>Saved plans</h3>{plans.slice(0, 5).map(plan => <div key={plan.id} className="rowline"><strong>{plan.destination}</strong><span>{new Date(plan.time).toLocaleString()}</span><p>{money(plan.profit)} trip · {money(plan.profitHour)}/h · {plan.tripTime}m</p></div>)}</div>; }
